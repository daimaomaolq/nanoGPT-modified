...
| 实验 | 参数 | 目的 |
| --- | --- | --- |
| no-KL | `kl_coef=0.0` | 验证 KL 对格式和漂移的影响。 |
...
```text
对齐训练：在 nanoGPT 上实现 mini-GRPO 强化学习对齐流程，包含 group rollout、rule-based reward、relative advantage、KL penalty 和无 critic 策略优化；构建 SFT-only、SFT-continued、GRPO-G4、GRPO-G8 等对照实验，在可自动判分算术任务上评估 accuracy、reward、format pass rate、invalid answer rate 和 KL。实验显示 GRPO 相比 SFT-only 在 hard arithmetic 上 accuracy 从 63.1% 提升至 65.2%，在 easy split 上从 70.0% 提升至 89.1%，并保持 100% format pass rate 与 0 invalid response；同时分析了 SFT-continued 强 baseline、reward 稀疏、group-size 消融和 GPU rollout 吞吐瓶颈。
```

如果简历篇幅有限，可压缩为：

```text
对齐训练：实现 mini-GRPO 对齐流程（group rollout、rule-based reward、relative advantage、KL penalty、no critic），并构建 SFT-only / SFT-continued / GRPO-G4 / GRPO-G8 消融；在可自动判分算术任务上，GRPO 相比 SFT-only 将 hard accuracy 从 63.1% 提升至 65.2%、easy accuracy 从 70.0% 提升至 89.1%，同时保持 100% format pass rate 和 0 invalid response，并复盘 reward 稀疏、group size 与 rollout 吞吐瓶颈。
```

更稳健、不夸大的版本：

```text
对齐训练：在 nanoGPT 中实现 mini-GRPO 强化学习对齐实验，覆盖 SFT warmup、group rollout、rule-based reward、relative advantage、KL penalty 与无 critic policy update；通过 SFT-only、SFT-continued、G4/G8 group-size 消融评估 reward、accuracy、format pass rate 与 KL，验证 GRPO 可稳定提升 SFT-only 并改善格式稳定性，同时分析其在有 gold answer 任务上未超过继续 SFT 的原因。
```
...
- 实现 mini-GRPO 代码闭环。
- 完成 SFT、GRPO、SFT-continued、G8 group-size 消融。
- 得到可解释的正向提升。
